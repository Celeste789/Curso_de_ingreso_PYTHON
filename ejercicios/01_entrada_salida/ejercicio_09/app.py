...
class App(customtkinter.CTk):
    ...
    def btn_mostrar_on_click(self):
        aumento = float(self.txt_incremento.get())
        sueldo = float(self.txt_sueldo.get())
        calculo = aumento * sueldo / 100
        sueldo += calculo
        mensaje = "El aumento es de {0} y el sueldo final es {1}".format(calculo, sueldo)
        alert(title="Aumento de sueldo", message=mensaje)
